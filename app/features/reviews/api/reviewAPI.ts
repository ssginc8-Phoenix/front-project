...
const host = 'https://beanstalk.docto.click/api/v1/reviews';
...
export async function getMyReviews(
  ...
  const url = 'https://beanstalk.docto.click/api/v1/users/me/reviews';
...
export async function getAllReviews(
  ...
  const url = 'https://beanstalk.docto.click/api/v1/admin/reviews';
...
export async function getHospitalReviews(
  ...
  const url = `https://beanstalk.docto.click/api/v1/hospitals/${hospitalId}/reviews`;
  const res = await axios.get(`${url}?page=${page}&size=${size}`, {
    withCredentials: true,
  });
  return res.data;
...
export async function reportReview(reviewId: number, reason: string): Promise<void> {
  await axios.post(`https://beanstalk.docto.click/api/v1/reviews/${reviewId}/report`, null, {
    params: { reason },
    withCredentials: true,
  });
}
...
export async function getReviewSummary(hospitalId: number): Promise<ReviewSummaryResponse> {
  const url = `https://beanstalk.docto.click/api/v1/hospitals/${hospitalId}/reviews/summary`;
